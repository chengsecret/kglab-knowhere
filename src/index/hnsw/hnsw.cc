...
class OODHnswIndexNode : public IndexNode {
    public:
    using DistType = float;
    OODHnswIndexNode(const int32_t& /*version*/, const Object& object) : index_(nullptr) {
    ...
    Train(const DataSetPtr dataset, std::shared_ptr<Config> cfg) override {
        ...
        auto hnsw_cfg = static_cast<const OODHNSWConfig&>(*cfg);
    ...
    std::vector<std::vector<float>> parseJsonToFloatArray(const std::string& jsonString) {
        // 解析JSON字符串
        ...
        for (const auto& item : json) {
            for (const auto& [key, value] : item.items()) {
                ...
                for (const auto& val : value) {
                    vectorData.push_back((float)val);
                }
    ...
    void fusionOOD(float* baseVecs, std::vector<std::vector<float>>& trainVecs,
        ...
        } else if (IsMetricType(metric_type, metric::COSINE)) {
            index = new faiss::IndexFlatIP(dim);
            faiss::fvec_renorm_L2(dim, train_rows, trains);
        } else {
 ...
 public:
    ...
    StaticCreateConfig() {
        return std::make_unique<OODHNSWConfig>();
    }
...
KNOWHERE_SIMPLE_REGISTER_DENSE_ALL_GLOBAL(OOD_HNSW, OODHnswIndexNode, knowhere::feature::MMAP | knowhere::feature::MV)
#endif
